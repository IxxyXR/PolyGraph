...
namespace Mixture
{
	[System.Serializable, NodeMenuItem("Poly For Start")]
	public class PolyForStart : ForeachStart
	{
		[Input]
		public ConwayPoly inputs;
		...
		public override string	name => "Poly For Start";
		...
		public HashSet<BaseNode> GatherNodesInLoop()
		{
			...
			while (l.Count > 0)
			{
				...
				if (!(node is PolyForStart))
					foreach (var i in node.GetInputNodes())
						l.Push(i);

				if (!(node is PolyForEnd))
